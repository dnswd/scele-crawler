...
it('throw error when login failed', async () => {
  ...
  await expect(sceleCrawler.login('abc', 'def')).to.be.rejectedWith('Login failed');
});
...
it('can get courses', () => {
  ...
  expect(courses[0].link).to.equal('https://scele.cs.ui.ac.id/course/view.php?id=1');
  ...
  expect(courses[1].link).to.equal('https://scele.cs.ui.ac.id/course/view.php?id=2');
  ...
  expect(courses[2].link).to.equal('https://scele.cs.ui.ac.id/course/view.php?id=3');
